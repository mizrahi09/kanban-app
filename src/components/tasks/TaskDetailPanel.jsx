...
// ── Rich text editor (contenteditable, supports inline image paste) ──
function RichEditor({ taskId, htmlContent, onSave }) {
  const editorRef = useRef(null)
  const [uploading, setUploading] = useState(false)

  // Set content only when switching tasks
  useEffect(() => {
  ...
  const insertImageAtCursor = (url) => {
    ...
    if (sel && sel.rangeCount) {
      ...
      editorRef.current.insertAdjacentHTML('beforeend', `<img src="${url}" />`)
    }
  }
  ...
  const handlePaste = async (e) => {
    ...
    setUploading(true)
    try {
      const sRef = storageRef(storage, `tasks/${taskId}/${Date.now()}.${file.type.split('/')[1] || 'png'}`)
  ...
  return (
    <div className="relative">
      {uploading && (
        <span className="absolute top-0 right-0 text-xs text-indigo-500 animate-pulse">Uploading…</span>
      )}
      <div
        ref={editorRef}
        data-rich-editor
        ...
        onPaste={handlePaste}
        data-placeholder="What is this task about? Paste an image to embed it inline."
        className="w-full text-sm text-gray-700 outline-none min-h-[80px] leading-relaxed"
      />
    </div>
  )
}
...
function AttachmentSection({ task, onUpdate }) {
  ...
  const handleFile = async (e) => {
    ...
    setUploading(true)
    try {
      const sRef = storageRef(storage, `tasks/${task.id}/${Date.now()}_${file.name}`)
      await uploadBytes(sRef, file)
      const url = await getDownloadURL(sRef)
      await onUpdate(task.id, { attachments: [...attachments, { url, name: file.name, type: file.type }] })
    } catch (err) {
          ...
          {attachments.map(a => (
            <div key={a.url} className="group relative">
              {a.type?.startsWith('image/') ? (
                ...
                <a href={a.url} target="_blank" rel="noreferrer"
                  className="flex items-center gap-1 text-xs text-indigo-600 border border-gray-200 rounded-lg px-2 py-1">
                  📄 {a.name}
                </a>
              )}
              <button
      ...
      <div className="flex-1 overflow-y-auto">
        ...
        <div className="px-6 pt-5 pb-3">
          <input
            ...
            onBlur={() => title.trim() !== task.title && save('title', title.trim() || 'Untitled task')}
            placeholder="Task title"
            className="w-full text-2xl font-bold text-gray-900 outline-none placeholder-gray-300 border-none bg-transparent"
          />
        </div>
        ...
        {/* Description — rich editor with inline image paste */}
        <div className="px-6 py-4 border-t border-gray-100">
        ...
        <div className="px-6 py-4 border-t border-gray-100">
          <p className="text-sm font-semibold text-gray-700 mb-3">
            Subtasks
            {subtasks.length > 0 && (
              <span className="text-gray-400 font-normal ml-1">({completedCount}/{subtasks.length})</span>
            )}
          </p>
          ...
          {showSubtaskInput ? (
            <div className="flex items-center gap-2 mt-2">
              <input
                ...
                onKeyDown={async e => {
                  if (e.key === 'Enter' && subtaskInput.trim()) { await addSubtask(subtaskInput); setSubtaskInput(''); setShowSubtaskInput(false) }
              ...
              />
              <button
                onClick={async () => { if (subtaskInput.trim()) { await addSubtask(subtaskInput); setSubtaskInput('') } setShowSubtaskInput(false) }}
            ...
            <button
              ...
              className="mt-2 text-sm text-gray-400 hover:text-indigo-600 flex items-center gap-1"
            >
          ...
          <div className="flex gap-3 items-start">
            ...
            <div className="flex-1">
              <textarea
                ...
                className="w-full text-sm text-gray-700 outline-none resize-none border border-gray-200 rounded-lg px-3 py-2 focus:ring-2 focus:ring-indigo-300 placeholder-gray-300 min-h-[60px]"
              />
              {commentText.trim() && (
                <button
                  onClick={handleCommentSubmit}
                  className="mt-1 text-xs bg-indigo-600 text-white px-3 py-1 rounded-lg hover:bg-indigo-700 float-right"
                >Send</button>
              )}
            </div>
          </div>
...
export default function TaskDetailPanel({ task, columns, boardId, onUpdate, onDelete, onClose }) {
  return (
